...
impl<'a> EncodingState<'a> {
    fn encode_core_modules(&mut self) {
        ...
        for (name, adapter) in self.info.adapters.iter() {
            ...
            let idx = if is_external {
                // For external modules, define a core module type and import
                // rather than embedding the module bytes.
                self.import_external_core_module(name, &adapter.wasm)
            } else if self.info.encoder.debug_names {
    ...
    /// Convert a library name to a valid kebab-case component import name.
    /// Handles standard libraries (`libc.so` → `c`) and path-based native
    /// extensions (`/1/numpy/core/_multiarray_umath.cpython-314-wasm32-wasi.so`
    ...
    /// Import an external core module by defining its type and creating a
    /// component-level import for it. Returns the core module index.
    fn import_external_core_module(&mut self, import_name: &str, wasm: &[u8]) -> u32 {
        let extern_name = Self::to_extern_name(import_name);
        use wasmparser::{Parser, Payload};

        // Parse the module to extract type information for imports and exports
        let mut func_types: Vec<wasmparser::FuncType> = Vec::new();
        let mut parsed_imports: Vec<(String, String, wasmparser::TypeRef)> = Vec::new();
        // Track the entity types for all defined items in index order
        // Functions: type index (from imports first, then function section)
        let mut func_type_indices: Vec<u32> = Vec::new();
        let mut tables: Vec<wasmparser::TableType> = Vec::new();
        let mut memories: Vec<wasmparser::MemoryType> = Vec::new();
        let mut globals: Vec<wasmparser::GlobalType> = Vec::new();
        let mut tags: Vec<wasmparser::TagType> = Vec::new();
        let mut parsed_exports: Vec<(String, wasmparser::ExternalKind, u32)> = Vec::new();

        // Count imports by kind to compute the index offset for defined items
        let mut _num_func_imports: u32 = 0;
        let mut num_table_imports: u32 = 0;
        let mut num_memory_imports: u32 = 0;
        let mut num_global_imports: u32 = 0;
        let mut num_tag_imports: u32 = 0;

        for payload in Parser::new(0).parse_all(wasm) {
            match payload.expect("valid wasm module") {
                Payload::TypeSection(reader) => {
                    for ty in reader.into_iter_err_on_gc_types() {
                        func_types.push(ty.expect("valid type"));
                    }
                }
                Payload::ImportSection(reader) => {
                    for import in reader.into_imports() {
                        ...
                        match import.ty {
                            ...
                            wasmparser::TypeRef::Tag(_) => { num_tag_imports += 1; }
                        }
                        parsed_imports.push((
                ...
                Payload::FunctionSection(reader) => {
                    for func in reader {
                        func_type_indices.push(func.expect("valid function"));
                    }
                }
                ...
                Payload::MemorySection(reader) => {
                    for mem in reader {
                        memories.push(mem.expect("valid memory"));
                    }
                }
                ...
                Payload::TagSection(reader) => {
                    for tag in reader {
                        tags.push(tag.expect("valid tag"));
                    }
                }
        ...
        // Build a ModuleType that faithfully represents the module's interface
        let mut module_type = wasm_encoder::ModuleType::new();

        // First, add all function types to the module type's type section
        for ft in &func_types {
            module_type.ty().function(
                ft.params().iter().map(|t| Self::convert_val_type(*t)),
                ft.results().iter().map(|t| Self::convert_val_type(*t)),
            );
        }

        // Add imports
        for (module, name, ty) in &parsed_imports {
            let entity = Self::convert_type_ref(ty, &func_types);
            module_type.import(module, name, entity);
        }

        // Add exports - resolve each export's entity type
        for (name, kind, index) in &parsed_exports {
            let entity = match kind {
                wasmparser::ExternalKind::Func | wasmparser::ExternalKind::FuncExact => {
                    let type_idx = func_type_indices[*index as usize];
                    EntityType::Function(type_idx)
                }
                wasmparser::ExternalKind::Table => {
                    let local_idx = *index - num_table_imports;
                    let ty = &tables[local_idx as usize];
                    Self::convert_table_type(ty)
                }
                wasmparser::ExternalKind::Memory => {
                    let local_idx = *index - num_memory_imports;
                    let ty = &memories[local_idx as usize];
                    Self::convert_memory_type(ty)
                }
                wasmparser::ExternalKind::Global => {
                    let local_idx = *index - num_global_imports;
                    let ty = &globals[local_idx as usize];
                    Self::convert_global_type(ty)
                }
                wasmparser::ExternalKind::Tag => {
                    let local_idx = *index - num_tag_imports;
                    let ty = &tags[local_idx as usize];
            ...
            module_type.export(name, entity);
        }

        // Define the core type and import the module
        let (type_idx, enc) = self.component.core_type(Some(&extern_name));
        enc.module(&module_type);

        self.component
            .import(&extern_name, ComponentTypeRef::Module(type_idx))
    }

    fn convert_type_ref(
        ty: &wasmparser::TypeRef,
        _types: &[wasmparser::FuncType],
    ) -> EntityType {
        match ty {
            wasmparser::TypeRef::Func(idx) | wasmparser::TypeRef::FuncExact(idx) => {
                EntityType::Function(*idx)
            }
            wasmparser::TypeRef::Table(ty) => Self::convert_table_type(ty),
            wasmparser::TypeRef::Memory(ty) => Self::convert_memory_type(ty),
            wasmparser::TypeRef::Global(ty) => Self::convert_global_type(ty),
    ...
    fn convert_table_type(ty: &wasmparser::TableType) -> EntityType {
        EntityType::Table(TableType {
            element_type: Self::convert_ref_type(ty.element_type),
            minimum: ty.initial.try_into().unwrap_or(0),
            maximum: ty.maximum.map(|m| m.try_into().unwrap_or(0)),
            table64: ty.table64,
            shared: false,
        })
    }

    fn convert_memory_type(ty: &wasmparser::MemoryType) -> EntityType {
    ...
    fn convert_global_type(ty: &wasmparser::GlobalType) -> EntityType {
        EntityType::Global(wasm_encoder::GlobalType {
            val_type: Self::convert_val_type(ty.content_type),
            mutable: ty.mutable,
            shared: ty.shared,
        })
    }
        ...
        wasm_encoder::RefType {
            ...
            heap_type: match ty.heap_type() {
                wasmparser::HeapType::Abstract { shared, ty } => {
                    match ty {
                        wasmparser::AbstractHeapType::Func => HeapType::Abstract { shared, ty: AbstractHeapType::Func },
                        wasmparser::AbstractHeapType::Extern => HeapType::Abstract { shared, ty: AbstractHeapType::Extern },
                        _ => HeapType::Abstract { shared, ty: AbstractHeapType::Func },
                    }
                }
    ...
    fn convert_val_type(ty: wasmparser::ValType) -> ValType {
        match ty {
            ...
            wasmparser::ValType::Ref(r) => ValType::Ref(Self::convert_ref_type(r)),
        }
    }
...
pub struct LibraryInfo {
    ...
    /// If true, this module will be imported as a core module rather than embedded.
    /// The module bytes are still used for link-time analysis but not included in the output.
    pub external: bool,
